...
Usage:
    manage.py (drive) [--model_SD=<model>] [--model_PK=<model>] [--model_YK=<model>] [--model_STOP=<model>] [--model_ARROW=<model>] [--js] [--type=(linear|categorical)] [--camera=(single|stereo)] [--meta=<key:value> ...] [--myconfig=<filename>]
...
def drive(cfg, model_path_SD=None,model_path_PK=None,model_path_YK=None,model_path_STOP=None,model_path_ARROW=None, use_joystick=False, model_type=None, camera_type='single', meta=[]):
    '''
    ...
    if camera_type == "stereo":
        ...
        V.add(cam, inputs=inputs, outputs=['cam/image_array'], threaded=threaded, run_condition = "camera")
    ...
    def add_model_path(model_path,subscript,V,model_type,cfg):
        ...
        if model_path:
            ...
            V.add(kl, inputs=inputs,
                outputs=outputs,
                run_condition=("run_pilot"+"_"+subscript))
            return V
        else:
            return V
    
    add_model_path(model_path_ARROW,"ARROW",V,"ARROW",cfg)  
    add_model_path(model_path_STOP,"STOP",V,"STOP",cfg)
    add_model_path(model_path_PK,"PK",V,"PK",cfg)
    add_model_path(model_path_YK,"YK",V,"YK",cfg)
    add_model_path(model_path_SD,"SD",V,model_type,cfg)    
...
if __name__ == '__main__':
    ...
    if args['drive']:
        ...
        drive(cfg, model_path_SD=args['--model_SD'],model_path_PK=args['--model_PK'],model_path_YK=args['--model_YK'],model_path_STOP=args['--model_STOP'],model_path_ARROW=args['--model_ARROW'], use_joystick=args['--js'],
