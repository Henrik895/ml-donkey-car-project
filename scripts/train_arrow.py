...
def core_cnn_layers(img_in, drop, l4_stride=1):
    ...
    """
    x = img_in
    x = conv2d(16, 5, (2,2), 1)(x)
    x = Dropout(drop)(x)
    x = conv2d(24, 5, (1,2), 2)(x)
    x = Dropout(drop)(x)
    x = conv2d(32, 5, (1,2), 3)(x)
    x = Dropout(drop)(x)
    x = conv2d(48, 3, (1,2), 4)(x)
    x = Dropout(drop)(x)
    x = conv2d(64, 3, (1,1), 5)(x)
    x = Dropout(drop)(x)
...
for tub in os.listdir('data_arrow_right2'):
    for image_file in os.listdir('data_arrow_right2/'+tub+'/images'):
        img = Image.open(f'data_arrow_right2/{tub}/images/{image_file}')
        img = np.asarray(img)
        img2 = np.copy(img[365:-315,380:-380,:])
        img2 = cv2.cvtColor(img2, cv2.COLOR_RGB2HSV)
        max_chan = np.max(img2[:,:,1])
        img2[img2[:,:,1] > max_chan-40] = 255
        img2[img2[:,:,1] <= max_chan-40] = 0
        #img2[:,:,0] = 0
        #img2[:,:,1] = 0
        #img2[img2[:,:,2] > 140] = 255
        #img2[:,:,2][img2[:,:,2] <= 140] = 0
...
for tub in os.listdir('data_arrow_left2'):
    for image_file in os.listdir('data_arrow_left2/'+tub+'/images'):
        img = Image.open(f'data_arrow_left2/{tub}/images/{image_file}')
        img = np.asarray(img)
        img2 = np.copy(img[365:-315,380:-380,:])
        img2 = cv2.cvtColor(img2, cv2.COLOR_RGB2HSV)
        max_chan = np.max(img2[:,:,1])
