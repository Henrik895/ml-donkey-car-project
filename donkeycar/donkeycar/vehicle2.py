...
class Vehicle:
    ...
    def preProcessImage(self,inputs,keyword):
        if(keyword == "STOP"):
        ...
        for entry in self.parts:
            ...
            if entry.get('run_condition'):
                ...
                if run_condition[0:3] == "run":
                    ...
                    if model_type != self.models[self.tick] and model_type != "SD":
            ...
            if run:
                ...
                if model_type == 'other':
                    ...
                    # run the part
                        print(model_type)
                        inputs[0] = self.preProcessImage(inputs[0], model_type)
                        if entry.get('thread'):
                            outputs = p.run_threaded(*inputs)
                        else:
                            outputs = p.run(*inputs)
                        if model_type == 'SD':
                            ...
                            if outputs[1] < 0.20:
                                ...
                                if self.throttleCounter >= 12: #This value needs to be tuned
                                    self.reverse = True
                                    self.reverseCounter = 6 #This value needs to be tuned
                            else:
                                self.cumulativeThrottle = 0
                ...
                #TODO, only self driving models output is given back
                if outputs is not None and model_type not in ['PK', 'YK', 'STOP']:
                    if model_type != "other":
                        #outputs = (outputs[0], -1*outputs[1])
                        if outputs[1] > 0.2:
                            #self.maxspeed += 0.0005
                            outputs = (outputs[0], np.minimum(outputs[1]*1.1, 0.80))
